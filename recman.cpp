...
//   if not available (or just deleted); create, and mark as seen
  ...
  */
  ...
  // add sorted by name
      ...
      if (all_recordings_sorted.begin() == all_recordings_sorted.end())
        dsyslog("live: RecordingsManager::update_all_recordings, after sort recordings, recycle_bin = %d, empty cont", recycle_bin);
      else
        dsyslog("live: RecordingsManager::update_all_recordings, after sort recordings, recycle_bin = %d, first = %s", recycle_bin, (*all_recordings_sorted.begin())->NameStr().c_str() );
    ...
    void update_all_recordings_scraper() {
      {
        LOCK_RECORDINGS_READ;
        update_all_recordings_scraper(Recordings, 0);
      }
#if VDRVERSNUM >= 20708
      {
        LOCK_DELETEDRECORDINGS_READ;
        update_all_recordings_scraper(DeletedRecordings, 1);
      }
#endif
    }
    void update_all_recordings_scraper(const cRecordings* Recordings, int recycle_bin) {
